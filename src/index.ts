...
const DIRECTORY_MARKER = ".cf-webdav-dir";
const DAV_ALLOW = "OPTIONS, HEAD, GET, PUT, DELETE, MKCOL, COPY, MOVE, PROPFIND";
const DAV_HEADERS: HeadersInit = {
  Allow: DAV_ALLOW,
  DAV: "1",
  "MS-Author-Via": "DAV",
};
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    const adminRoute = parseAdminRoute(url.pathname, env.ADMIN_TOKEN);
    if (adminRoute) {
...
function parseAdminRoute(pathname: string, adminToken?: string): { basePath: string; subPath: string } | null {
  if (!adminToken) {
    return null;
  }

  const prefix = `/${MANAGE_SEGMENT}/${adminToken}`;
  if (pathname === prefix) {
...
function parseAppRoute(pathname: string): { token: string; basePath: string; logicalPath: string } | null {
...
async function handleAdminRequest(
  ...
  env: Env,
  url: URL,
  route: { basePath: string; subPath: string },
): Promise<Response> {
  if (route.subPath === "/") {
    if (request.method !== "GET") {
      return adminMethodNotAllowed();
    }
    return new Response(renderAdminPage(url.origin, route.basePath), {
      status: 200,
      headers: new Headers({
  ...
  if (route.subPath === "/api/apps") {
    switch (request.method) {
      case "GET":
        return jsonResponse({
          apps: await listApps(env, url.origin),
        });
      case "POST":
...
async function handleWebDavRequest(
  ...
  if (!(await isAuthorizedForApp(request, access))) {
    return unauthorized(access.appName);
  }
...
async function handleCreateApp(request: Request, env: Env, origin: string): Promise<Response> {
  ...
  try {
    payload = (await request.json()) as AppPayload;
  } catch {
...
async function handleDeleteApp(request: Request, env: Env, appId: string): Promise<Response> {
  ...
  if ((request.headers.get("content-length") ?? "0") !== "0") {
    try {
      const payload = (await request.json()) as { purgeData?: unknown };
  ...
  await env.WEBDAV_CONFIG.delete(`${APP_KEY_PREFIX}${existing.id}`);
  await env.WEBDAV_CONFIG.delete(`${TOKEN_KEY_PREFIX}${existing.slug}`);
...
function adminMethodNotAllowed(): Response {
...
function adminErrorResponse(code: AdminErrorCode, status: number): Response {
  return jsonResponse(
    {
...
function adminErrorMessage(code: AdminErrorCode): string {
  switch (code) {
    case "invalid_json":
...
async function isAuthorizedForApp(request: Request, access: AppAccess): Promise<boolean> {
...
function directoryMarkerKey(collectionKey: string): string {
  return `${stripTrailingSlash(collectionKey)}/${DIRECTORY_MARKER}`;
}
...
async function handleCopyMove(
  ...
  if (destinationUrl.host !== url.host) {
    return new Response("Cross-host copy or move is not supported.", {
      status: 502,
      headers: baseHeaders(),
    });
  }

  const destinationRoute = parseAppRoute(destinationUrl.pathname);
  if (!destinationRoute || destinationRoute.token !== access.slug) {
  ...
  const destinationKey = normalizeKey(destinationRoute.logicalPath, source.kind === "collection");
  ...
  const existingDestination = await getResource(env, destinationRoute.logicalPath, access);
...
async function collectStorageKeys(env: Env, prefix: string): Promise<string[]> {
...
async function deleteStorageKeysInBatches(env: Env, keys: string[]): Promise<void> {
...
async function listApps(env: Env, origin: string): Promise<Array<ReturnType<typeof publicApp>>> {
  ...
  return apps
    .filter((app): app is ReturnType<typeof publicApp> => Boolean(app))
    .sort((left, right) => left.name.localeCompare(right.name, "zh-CN"));
}

async function getAppById(env: Env, id: string): Promise<AppRecord | null> {
  const raw = await env.WEBDAV_CONFIG.get<Record<string, unknown>>(`${APP_KEY_PREFIX}${id}`, "json");
  return hydrateAppRecord(raw);
}

async function getAppBySlug(env: Env, slug: string): Promise<AppRecord | null> {
  const raw = await env.WEBDAV_CONFIG.get<Record<string, unknown>>(`${TOKEN_KEY_PREFIX}${slug}`, "json");
  return hydrateAppRecord(raw);
}

function publicApp(app: AppRecord, origin: string) {
  return {
    id: app.id,
    name: app.name,
    slug: app.slug,
...
function hydrateAppRecord(raw: Record<string, unknown> | null): AppRecord | null {
...
function buildAppUrl(origin: string, slug: string): string {
  return `${origin}/${slug}/`;
}

async function validateAppUniqueness(
  env: Env,
  record: AppRecord,
  currentId?: string,
): Promise<AdminErrorCode | null> {
  const existingBySlug = await env.WEBDAV_CONFIG.get<AppRecord>(`${TOKEN_KEY_PREFIX}${record.slug}`, "json");
  if (existingBySlug && existingBySlug.id !== currentId) {
    return "path_in_use";
  }

  const allApps = await listApps(env, "https://placeholder.invalid");
  const duplicatePrefix = allApps.find((app) => app.rootPrefix === record.rootPrefix && app.id !== currentId);
  if (duplicatePrefix) {
...
async function saveApp(env: Env, record: AppRecord, previous?: AppRecord): Promise<void> {
...
function normalizeAuthUsername(
  input: unknown,
): { ok: true; value: string } | { ok: false; errorCode: AdminErrorCode } {
  if (input === undefined || input === null) {
    return { ok: true, value: "" };
...
async function ensureCollectionExists(env: Env, rootPrefix: string): Promise<void> {
  const markerKey = directoryMarkerKey(rootPrefix);
...
function renderAdminPage(origin: string, adminBasePath: string): string {
  return `<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8" />
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    @media (max-width: 900px) {
      ...
      .shell { width: min(100vw - 16px, 1160px); margin-top: 16px; }
...
<body>
  <div class="shell">
    <div class="topbar">
      ...
      <div class="top-actions">
        ...
        <div class="badge">${origin}${adminBasePath}</div>
      </div>
    </div>
  ...
  <script>
    const apiBase = ${JSON.stringify(adminBasePath + "/api")};
    ...
    const renderStatus = () => {
      ...
      statusEl.className =
        "status" + (message ? " show" : "") + (statusState && statusState.isError ? " error" : "");
    };

    const setStatusKey = (key, isError = false, params) => {
      statusState = key ? { mode: "key", key, isError, params: params || {} } : null;
      renderStatus();
    };

    const setStatusText = (message, isError = false) => {
      statusState = message ? { mode: "text", message, isError } : null;
      renderStatus();
    };
    ...
    const loadApps = async () => {
      const response = await fetch(apiBase + "/apps", { cache: "no-store" });
      const data = await response.json();
      if (!response.ok) {
